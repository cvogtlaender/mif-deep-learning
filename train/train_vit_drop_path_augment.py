...
from models.vit_drop_path import ViTDropPath
...
def main():
    ...
    model = ViTDropPath(num_classes=101, pretrained=True, drop_path_rate=0.1).to(device)
    
    criterion = nn.CrossEntropyLoss()
    model, history = train_model(
        model, train_loader, val_loader, device, criterion=criterion, num_epochs=20, 
        lr=1e-4, early_stopping_patience=10, use_mixup=True, num_classes=101)
